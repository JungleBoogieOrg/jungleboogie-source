...
connected({msg, Msg}, _From, {State, OtherState}) when is_record(State, zkstate) ->
	...
	case DecHdr#'RequestHeader'.type of
		0 ->
			...
			ResponseBin = none;
		1 ->
		    {DecReq, _Rest2} = org_apache_zookeeper_proto:read_CreateRequest(Rest1),
	...
	{reply, {{DecHdr, DecReq}, ResponseBin}, connected, {State#zkstate{zxid = State#zkstate.zxid + 1}, OtherState}}. 
...
create_resp(ReqHdr, CreateReq, Zxid) when is_record(CreateReq, 'CreateRequest') ->
    ...
    % Create a reply header appropriately....
